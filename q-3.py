# -*- coding: utf-8 -*-
"""TensorFlow_aish.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dgvIBSFWnV69hq1RbNA1BPFj572yRwZP
...
#### What is it?
TensorFlow is a system for executing computational graphs over Tensor objects, with native support for performing backpropogation for its Variables. In it, we work with <b>Tensors which are n-dimensional arrays analogous to the numpy ndarray.</b>
...
def three_layer_convnet(x, params):
    ...
    ############################################################################
#     added padding to X
    x_with_padding = tf.pad(x,[[0,0],[2,2],[2,2],[0,0]],'CONSTANT')
#   apply convolution with filter
    conv1 = tf.nn.conv2d(x_with_padding,conv_w1,[1,1,1,1],padding='VALID')+conv_b1
#     apply activation function ReLU
    conv_relu1 = tf.nn.relu(conv1)
    x_padded_1 = tf.pad(conv_relu1,[[0,0],[1,1],[1,1],[0,0]],'CONSTANT')
    conv2 = tf.nn.conv2d(x_padded_1,conv_w2,[1,1,1,1],padding='VALID')+conv_b2
    #     apply activation function ReLU
    conv_relu2 = tf.nn.relu(conv2)
    fc_x = flatten(conv_relu2)
...
def three_layer_convnet_init():
    ...
    ############################################################################
    wt_1 = tf.Variable(kaiming_normal((5,5,3,6)))
    bias_1 = tf.Variable(kaiming_normal((1,6)))
    wt_2 = tf.Variable(kaiming_normal((3,3,6,9)))
    bias_2 = tf.Variable(kaiming_normal((1,9)))
    w = tf.Variable(kaiming_normal((32 * 32 * 9,10)))
    b = tf.Variable(kaiming_normal((1,10)))
    params = [wt_1,bias_1,wt_2,bias_2,w,b]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return params
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        ########################################################################
        initializer = tf.variance_scaling_initializer(scale=2.0)
        self.conv1 = tf.layers.Conv2D(filters = channel_1,kernel_size = [5,5], strides = [1,1],padding = 'SAME',activation = tf.nn.relu,
                                     use_bias = True,kernel_initializer = initializer, bias_initializer = initializer,name = 'conv1')
